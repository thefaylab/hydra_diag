...
```{r}
for (survey in unique(survey_obs$survey)) {
  cat("##### Survey #", survey, " \n")
  p1 <- survey_obs %>% 
...
##### Size composition of surveys by species {.tabset}
...
for (sp in especies) {
  ...
  plot_diet[[sp]] <-  pred_obs%>% filter(species == sp & number==1) %>%
...
##### Proportions by weight for stomach by predator size bin {.tabset}
